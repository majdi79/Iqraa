  ...
  if (authLoading) {
    return (
      <div dir="rtl" className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-emerald-900 font-medium">جاري تحميل النظام...</div>
      </div>
    );
  }
...
function IqraaApp({ user }: { user: User }) {
  ...
  const handleSessionSubmit = (sessionData: any) => {
    ...
    setView(activeStudent ? 'student_profile' : 'dashboard');
  };
  ...
  const handleOpenSchedule = (student?: Student) => {
    setScheduleStudent(student || activeStudent || null);
    setIsScheduleModalOpen(true);
  };

  const handleSaveSchedule = (
    studentId: string,
    schedule: { nextSessionDate?: string; nextSessionTime?: string; nextSessionNotes?: string }
  ) => {
  ...
  if (loading) {
    return (
      <div dir="rtl" className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-emerald-900 font-medium">جاري تحميل البيانات...</div>
      </div>
    );
  }
    ...
    <div dir="rtl" className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <main className="flex-1 py-6">
        ...
        {view === 'student_profile' && activeStudent && (
          <StudentProfile
            student={students.find((student) => student.id === activeStudent.id) || activeStudent}
            sessions={sessions}
      ...
      <footer className="text-center text-xs text-slate-400 py-3">{user.email}</footer>
    </div>
  );
}
